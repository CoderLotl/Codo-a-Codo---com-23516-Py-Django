...
async function AutoSearchByReceivedParams()
{
    try
    {
        let consola = localStorage.getItem('console');
        if(consola)
        {            
            let games = await dataAccess.RetrieveGamesByConsole(urlTarget, consola);
            return games;
        }        
    }
    catch(error)
...
function DrawGameArticle(game, displayContainer)
{
    ...
    itemImg.src = `static/core/Data/consoles/${game.console}/${game.folder}/${game.folder}1Principal.jpg`;
    ...
    {
        const gameData = [{ name: game.name, SKU: game.SKU }];
